...
//#endif
  ...
  setTitle: function pdfViewSetTitle(title) {
    ...
    event.initCustomEvent("PDFViewerTitle", true, true, title);
    window.dispatchEvent(event);
//#if B2G
...
//#endif
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    PDFJS.Promise.all([firstPagePromise, storePromise]).then(function() {
      var storedHash = null;
      if (!IGNORE_STORED_HASH && store.get('exists', false)) {
...
var PageView = function pageView(container, id, scale,
  ...
  this.draw = function pageviewDraw(callback) {
    ...
    var bbLayer = this.bbLayer =
          bbLayerDiv ? new BoundingBoxLayerBuilder(bbLayerDiv, this.id - 1, canvas.width, canvas.height, this.viewport, pdfPage.commonObjs, pdfPage.objs)
                     : null;
...
//#if !(FIREFOX || MOZCENTRAL)
  var file = params.file || evt.detail || DEFAULT_URL;
//#else
